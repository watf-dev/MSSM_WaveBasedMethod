    ...
    for kk = 1:4
      ...
      switch cBC
        ...
        case 'v'
          ...
          % evaluate Shape Function at Gauss Point coordinates
          Psi_ii = evalShapeFunction( ...
                     kxy(ii,1),kxy(ii,2), ...
                     Lx,Ly, ...
                     cXGP,cYGP,set_ii);
               
          % evaluate Shape Function Derivatives as vector quantity
          ...
          % normal derivative along edge normal nVec
          % - ™ΩΩu√‰°GnVec ¨O 1x2
          % - B-spline √‰°GnVec ¨O nGP x 2°]®C≠” Gauss ¬I§@≠” normal°^
          Psidot_jj = sum(Psidot_jj_vec .* nVec', 1);
                         
          % numerical integration (vector scalar product)
          K(ii,jj)  = (1i/(density*omega) * ...
                       (cWGP.*Psi_ii)) * Psidot_jj.' + K(ii,jj);
  ...
  for kk = 1:4
    ...
    switch cBC
      ...
      case 'v'
        ...
        % at the Gau? Point coordinates
        Psi_ii = evalShapeFunction(kxy(ii,1),kxy(ii,2),...
               Lx,Ly,...
               cXGP,cYGP,set_ii);
             
        % evaluate load function derivative at Gau? Points     
      ...
      case 'p'
        ...
        % evaluate load function at Gau? Points     
...
switch config
  ...
  case "Bspline"
    etaLine = linspace(-1,1,150);
    [Xi,Eta] = meshgrid(etaLine, etaLine);
    xL = zeros(size(etaLine));
    yL = (etaLine+1)/2;
    [xR,yR,~] = calcGPcoordinatesBspline(nodes{2},p,etaLine.',1);
...
figure('Name','Total Solution Visualization','NumberTitle','off');
...
function [xGP_,yGP_,wGP_] = calcGPcoordinatesBspline(cps,p,xhi,w)
  ...
  for xhi_index = 1:size(xhi,1)
    ...
    dN = zeros(1,nn);
    for i = 1:nn
      dN(i) = bsplineBasisDerivative(i,p,kv,xhi(xhi_index));
    end
    dC = dN * cps;               
    Lt(xhi_index) = sqrt(dC(1)^2 + dC(2)^2);  % |dC/du|
