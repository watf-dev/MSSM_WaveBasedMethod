...
% #########################################################################
%                       
%             WAVE BASED METHOD - INTRODUCTION EXAMPLE
%
%                Project for Dissertation - 2019
%
%
% *************************************************************************
%
%   Chair of Strucutural Mechanics at the department of Civil Engineering 
%
%                       at TUM - Prof. Mueller
%
% *************************************************************************
% 
% written by:   Hannes Englert  
%
% #########################################################################
...
% refers to maximal dimension in x- and y-direction 

Lx = 20;
Ly = 10;
      
%  *************************************
%  ***  II.2 Properties and Material ***
%  *************************************
...
%  ***********************************************************
%  ***  II.4 Shape Function Sets and Numerical Integration ***
%  ***********************************************************

% truncation factors of each wave function set
n1 = 30;
n2 = 30;

% number of GP per edge
% possible values nGP =  1:11,15,50,100
nGP = 50;                 
% GP_coordinates = 0;
% GP_weighting   = 0; 

%  ******************************
%  ***  II.5 External loading ***
%  ******************************
...
%  ********************************************************************
%  ***  III.2 Define Shape Functions and Shape Function Derivatives ***
%  ********************************************************************

% see Shape Function Definition at end of file
...
% Calculate Normal Vectors on edges 
tVecEdges = zeros(4,2);
nVecEdges = zeros(4,2);
...
for ii = 1:4
    ...
    %      | t2 |
    %
    tVecEdges(ii,1:2) = [endPoint(1) - startPoint(1),...    % x-coordinate
                           endPoint(2) - startPoint(2)];    % y-coordinate
    % length for normalization   
    %       ___________
    % l = \/ x^2 + y^2 
    %
    cLtVec            = abs(sqrt(tVecEdges(ii,1)^2+tVecEdges(ii,2)^2));
    % normal vector 
    %
    %      | n1 |   |  t2 |
    %  n = |    | = |     |
    %      | n2 |   | -t1 |
    %
    nVecEdges(ii,1:2) = [tVecEdges(ii,2)/cLtVec,-tVecEdges(ii,1)/cLtVec];
    
end

% GP coordinates and weighting factors
xGP = zeros(4,nGP);
yGP = zeros(4,nGP);
wGP = zeros(4,nGP);
...
for ii = 1:4
    ...
    [xGP_,yGP_,wGP_] = getGPcoordinates(startPoint,endPoint,nGP);
...
% *************************************************************************
% 
% Description:  Wiki  % todo: where is this wiki ?
%
% #########################################################################

function [xGP,yGP,wGP] = getGPcoordinates(startPoint,endPoint,nGP)

% get GPcoordinates and weights for local convective coordinate system
% xhi = -1..1
...
length = abs( sqrt( ( endPoint(1)- startPoint(1) )^2 + ...
